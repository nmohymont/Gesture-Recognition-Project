...
def plot_xy_grid_corrige(df, df_downsampled, subject=1, line_color='black'):
    ...
    for row_idx, digit in enumerate(Digits):
        ...
        rep_counts = df_group.groupby("repetition").size()
        best_rep = rep_counts.idxmin()

        # données originales pour la répétition choisie
        df_orig = df_group[
            df_group['repetition'] == best_rep
        ].sort_values('t').copy()

        n_orig = len(df_orig)

        # données sous-échantillonnées pour le même groupe
        # IMPORTANT : on utilise df_down pour ne pas écraser df_downsampled
        df_down = df_downsampled[
            (df_downsampled['subject_id'] == subject) &
            (df_downsampled['digit'] == digit) &
            (df_downsampled['repetition'] == best_rep)
        ].sort_values('t').copy()
        ...
        for col in ["x", "y"]:
            df_orig[col] = df_orig[col].astype(float)
            df_down[col] = df_down[col].astype(float)

        # Limite d'axes
        x_min = min(df_orig["x"].min(), df_down["x"].min())
        x_max = max(df_orig["x"].max(), df_down["x"].max())
        y_min = min(df_orig["y"].min(), df_down["y"].min())
        y_max = max(df_orig["y"].max(), df_down["y"].max())
        margin = 0.05
        ...
        ax_right.set_title(
            f"Digit {digit} — Downsampled [n = {n_downsampled}]",
            fontsize=9,
            fontweight="bold",
            loc="left"
        )
